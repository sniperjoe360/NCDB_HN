...
# exclude those with unknown radiation
...
(
	(dats2$RAD_REGIONAL_RX_MODALITY == 31 & dats2$RX_SUMM_CHEMO != 0 & dats2$RX_SUMM_CHEMO < 82) & 
	(
		dats2$RX_SUMM_SURG_PRIM_SITE < 30 | dats2$RX_SUMM_SURG_PRIM_SITE == 99 |
		((dats2$R_Date + 50 ) < dats2$S_Date) & !is.na(dats2$R_Date) & !is.na(dats2$S_Date)
	)
)
,]
dim(dats3)
