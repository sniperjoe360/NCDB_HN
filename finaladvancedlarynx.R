...
(#read$TNM_CLIN_T == 1 | read$TNM_CLIN_T == '1A' | read$TNM_CLIN_T == '1B' |
read$TNM_CLIN_T == 2 | read$TNM_CLIN_T == 3)
 
&
 
!is.na(read$TNM_CLIN_T)
...
!(dat$TNM_CLIN_T == '1A' & dat$TNM_CLIN_N == 0)
 
&
 
!(dat$TNM_CLIN_T == '1A' & dat$TNM_CLIN_N == 1)
 
&
!(dat$TNM_CLIN_T == '1B' & dat$TNM_CLIN_N == 0)
 
&
 
!(dat$TNM_CLIN_T == '1B' & dat$TNM_CLIN_N == 1)
 
&
!(dat$TNM_CLIN_T == 1 & dat$TNM_CLIN_N == 0)
 
&
 
!(dat$TNM_CLIN_T == 1 & dat$TNM_CLIN_N == 1)
 
&
 
!(dat$TNM_CLIN_T == 2 & dat$TNM_CLIN_N == 0)
 
&
 
!(dat$TNM_CLIN_T == 2 & dat$TNM_CLIN_N == 1)
 
&
 
!(dat$TNM_CLIN_T == 3 & dat$TNM_CLIN_N == 0)
...
dat4 <- dat2[dat2$TNM_CLIN_M != 1,]
dim(dat4)
# only supraglottic larynx
#dat5 <- dat4[dat4$PRIMARY_SITE == 'C320',]
 
# Exclude patients with previous cancer history
dats2A <- dat4[dat4$SEQUENCE == 1 |
     dat4$SEQUENCE == 0
,]
dim(dats2A)
 
# Exclude patients unknown radiation modality
dats2 <- dats2A[!is.na(dats2A$RAD_REGIONAL_RX_MODALITY)
,]
dim(dats2)
 
# timing
dats2$R_Date <- dats2$DX_RAD_STARTED_DAYS
dats2$S_Date <- dats2$DX_SURG_STARTED_DAYS
dats2$C_Date <- dats2$DX_CHEMO_STARTED_DAYS
dats2$R_Leng <- dats2$RAD_ELAPSED_RX_DAYS
 
# Include patients with defintive CRT vs.  or total laryngectomy 
# Include patients with defintive CRT and salvage vs.  or total laryngectomy 
dats3 <- dats2[
(
                (dats2$RX_SUMM_SURG_PRIM_SITE >= 40 & dats2$RX_SUMM_SURG_PRIM_SITE < 50) &
                (
                                 dats2$RX_SUMM_RADIATION == 0     |
dats2$RX_SUMM_SURGRAD_SEQ == 3 
                )
)
...
(
                (dats2$RAD_REGIONAL_RX_MODALITY == 31 & !is.na(dats2$C_Date) & dats2$RX_SUMM_SURG_PRIM_SITE == 0) 
...
dat8 <- dat7[(dat7$DX_LASTCONTACT_DEATH_MONTHS < 6 & (dat7$PUF_VITAL_STATUS != 0))|dat7$DX_LASTCONTACT_DEATH_MONTHS >= 6 ,]
dim(dat8)
...
larynx[(
                (larynx$RAD_REGIONAL_RX_MODALITY == 31 & !is.na(larynx$C_Date) & larynx$RX_SUMM_SURG_PRIM_SITE == 0) 
 
) ,'treatment'] = '2. Larynx Preservation'
 
larynx[(
                (larynx$RX_SUMM_SURG_PRIM_SITE >= 40 & larynx$RX_SUMM_SURG_PRIM_SITE < 50) &
                (
                                 larynx$RX_SUMM_RADIATION == 0     |
larynx$RX_SUMM_SURGRAD_SEQ == 3
                )
),'treatment'] = '1. Total Laryngectomy +/- CRT/RT'
...
larynx_CRT <- larynx[(larynx$treatment_Bin == FALSE ),]
larynx_TL <- larynx[(larynx$treatment_Bin == TRUE ),]
...
hist(larynx_CRT$C_Date,100,col='blue',main='Initiation Time of Treatment for T1-T3 N+ Larynx who had CRT \n Green = Surgery, Red = RT, Blue = Chemo',add=F)
hist(larynx_CRT$R_Date,100,col='red',add=T)
hist(larynx_CRT$S_Date,100,col='green',add=T)
...
library(MatchIt)
# select variables
myvars <- c('SEX','treatment_Bin','treatment',"AGE_Recode","YEAR_OF_DIAGNOSIS_Recode","TNM_CLIN_T_Recode","TNM_CLIN_N_Recode","RACE_Recode","GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'CDCC_TOTAL_Recode','RX_SUMM_CHEMO_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode','followup_time','censor','HISPANIC_Recode','EDUCATION_Recode','INCOME_Recode','INSURANCE_STATUS_Recode','RAD_REGIONAL_RX_MODALITY','HISTOLOGY_Recode')
...
listVars<-(c('SEX',"AGE_Recode","RACE_Recode","YEAR_OF_DIAGNOSIS_Recode","TNM_CLIN_T_Recode",'TNM_CLIN_N_Recode',"GRADE_Recode","HISTOLOGY_Recode","RAD_REGIONAL_RX_MODALITY","RX_SUMM_CHEMO_Recode",'CDCC_TOTAL_Recode','EDUCATION_Recode','INCOME_Recode','INSURANCE_STATUS_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode'))
 
catVars<-(c('SEX',"AGE_Recode","RACE_Recode","YEAR_OF_DIAGNOSIS_Recode","TNM_CLIN_T_Recode",'HISTOLOGY_Recode','TNM_CLIN_N_Recode',"RAD_REGIONAL_RX_MODALITY","RX_SUMM_CHEMO_Recode","GRADE_Recode",'CDCC_TOTAL_Recode','FACILITY_TYPE_CD_Recode','EDUCATION_Recode','INCOME_Recode','INSURANCE_STATUS_Recode','CROWFLY_Recode'))
 
table1 <- CreateTableOne(vars = listVars, data = larynx_test, factorVars = catVars, strata = 'treatment',includeNA=TRUE)
print(table1)
write.csv(print(table1),'H:\\NCDB Files\\table1_matched_larynx.csv')
############################## KM
...
plot(subset, xlab="Months", ylab="Survival Probability (%)", main='Overall Survival of All T2N2-3 and T3N+ Larynx \n Stratified by treatment',lwd=2,col=c(1:length(names(subset$strata))),lty=1,cex.axis = 2,cex.lab = 1.5,cex.main = 1.5)
legend(70, 1.0, c("IMRT+Chemotherapy","TL +/- CRT/RT"),lwd=4,col=c(1:length(names(subset$strata))),lty=1)
...
library(survminer)
ggsurvplot(subset, main='  Overall Survival of All T2N2-3 and T3N+ Larynx \n                    Stratified by treatment',xlab='Months',pval = TRUE,
...
#           risk.table.col = "strata",
           ...
           ggtheme = theme_bw(),
           legend.labs = c( "Upfront CRT","TL +/- CRT/RT"))
...
coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(treatment),data=larynx_test)
...
coxmodel <- coxph(Surv(followup_time, censor)~ CROWFLY_Recode,data=larynx_test)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),round(coef(summary(coxmodel))[,5],2)))
  
coxmodel <- coxph(Surv(followup_time, censor)~ FACILITY_TYPE_CD_Recode,data=larynx_test)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),round(coef(summary(coxmodel))[,5],2)))
...
write.csv(UNI_Results,'H:/NCDB Files\\UNIRESULTS_matched.csv')

# significant variables: treatment, sex. age. N stage, CDCC, income, insurance
