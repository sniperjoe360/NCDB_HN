...
plot(allpts, xlab="Time", ylab="Survival Probability (%)",lwd=2, main="Kaplan Meier Plot for Survival of All T2-3N<3M0 Larynx")
...
write.csv(round(finalCox,2),file="H:/NCDB Files/CCRTCoxT3N0_.csv")
...
logrank <- survdiff(Surv(followup_time, censor)~IMRT)
pval <- 1 - pchisq(logrank$chisq, length(logrank$n) - 1)
text(30,0.07,paste("IMRT vs. Other logrank ","p = ",pval,sep = ' '))
