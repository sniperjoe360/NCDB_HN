...
# Include patients with defintive CRT and salvage vs. partial or total laryngectomy alone 
dats3 <- dats2[
(
	(dats2$RX_SUMM_SURG_PRIM_SITE >= 40 & dats2$RX_SUMM_SURG_PRIM_SITE <= 50) & 
	(
		 dats2$DX_SURG_STARTED_DAYS < dats2$DX_RAD_STARTED_DAYS | 
		 dats2$RX_SUMM_RADIATION == 0     | 
		 is.na(dats2$DX_RAD_STARTED_DAYS) | 
		 (
			is.na(dats2$DX_SURG_STARTED_DAYS) & !is.na(dats2$DX_RAD_STARTED_DAYS)
		 )
	)
) 
...
(
	(dats2$RAD_REGIONAL_RX_MODALITY == 31 & !is.na(dats2$C_Date)) 
	& 
	(dats2$RX_SUMM_SURG_PRIM_SITE == 0 | (((dats2$R_Date + 50 ) < dats2$S_Date) & is.na(dats2$S_Date) & is.na(dats2$R_Date)))
...
larynx[(
	(larynx$RAD_REGIONAL_RX_MODALITY == 31 & !is.na(larynx$C_Date)) 
	& 
	(larynx$RX_SUMM_SURG_PRIM_SITE == 0 | (((larynx$R_Date + 50 ) < larynx$S_Date) & is.na(larynx$S_Date) & is.na(larynx$R_Date)))
) ,'treatment'] = 'CRT'

larynx[(
	(larynx$RX_SUMM_SURG_PRIM_SITE >= 40 & larynx$RX_SUMM_SURG_PRIM_SITE <= 50) & 
	(
		 larynx$DX_SURG_STARTED_DAYS < larynx$DX_RAD_STARTED_DAYS | 
		 larynx$RX_SUMM_RADIATION == 0     | 
		 is.na(larynx$DX_RAD_STARTED_DAYS) | 
		 (
			is.na(larynx$DX_SURG_STARTED_DAYS) & !is.na(larynx$DX_RAD_STARTED_DAYS)
		 )
	)
) ,'treatment'] = 'TL'
...
larynx_CRT <- larynx[(larynx$treatment_Bin == TRUE ),]
larynx_TL <- larynx[(larynx$treatment_Bin == FALSE ),]
...
+ as.factor(TNM_CLIN_N_Recode), data = matchdata, method = "nearest");
#summary(m.out);
...
legend(70, 1.0, c("Concurrent IMRT + Chemotherapy +/- Salvage","TL +/- CRT/RT"),lwd=4,col=c(1:length(names(subset$strata))),lty=1)
...
#           risk.table.col = "strata",
           ...
           ggtheme = theme_bw(),
           legend.labs = c( "CRT +/- Salvage","TL +/- CRT/RT"))
